...
func hasKernelModule(name string) (bool, error) {
	...
	procMod, err := os.Open("/proc/modules")
	if err != nil {
		return false, fmt.Errorf("module %s not found in /sys/module or /proc/modules: %w", name, err)
	}
	defer procMod.Close()
	...
	copy(prefix, name)
	prefix[len(prefix)-1] = ' '
	scanner := bufio.NewScanner(procMod)
	for scanner.Scan() {
...
// findExecutable checks paths for a path that exists and is executable by the current user.
...
// run, it instead falls back to a syscall interface to attemtp to install a
// module.
func tryInstallModule(name string) error {
	path := findExecutable([]string{"/usr/sbin/modprobe", "/sbin/modprobe"})
	if path != "" {
